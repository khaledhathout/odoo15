...
class PurchaseReturnReportWizard(models.TransientModel):
    ...
    def get_report(self):
        ...
        return self.env.ref('wedo_purchase_return_managment.purchase_return_report').report_action(self, data=data)
    ...
    def get_report_xlsx(self):
        ...
        return self.env.ref('wedo_purchase_return_managment.purchase_return_report_xlsx').report_action(self, data=data)


class PurchaseReturnReport(models.AbstractModel):
    _name = 'report.wedo_purchase_return_managment.purchase_return_template'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        docs = self.env['purchase.return.line'].search([('date_order', '>=', from_date), ('date_order', '<=', to_date),
                                                       ('order_id.state', '=', 'done'), ('order_id.company_id', '=', self.env.company.id)])
...
class PurchaseReturnReportXLSX(models.AbstractModel):
    _name = 'report.wedo_purchase_return_managment.purchase_r_template_xlsx'
    _inherit = "report.report_xlsx.abstract"
    _description = "Purchase Return Report XLSX"
    ...
    def generate_xlsx_report(self, workbook, data, partners):
        ...
        docs = self.env['purchase.return.line'].search(
        ...
        sheet.merge_range('H8:K9', "Items Purchases Returns", header_format3)
        ...
        sheet.write(12, 1, "PO Return Order", table_header_format)
        sheet.write(12, 2, "PO Return State", table_header_format)
        ...
        sheet.write(12, 8, "PO Return Date", table_header_format)
        ...
        sheet.write(12, 24, "Price", table_header_format)
        sheet.write(12, 25, "Amount Before Discount", table_header_format)
        sheet.write(12, 26, "Discount Amount", table_header_format)
        sheet.write(12, 27, "Amount After Discount", table_header_format)
        sheet.write(12, 28, "Tax Amount", table_header_format)
        sheet.write(12, 29, "Total Amount", table_header_format)
        sheet.write(12, 30, "LM", table_header_format)
        sheet.write(12, 31, "M2", table_header_format)
        sheet.write(12, 32, "PO Return Responsible", table_header_format)
        ...
        for doc in docs:
            ...
            col += 1
            sheet.write(row, col, doc.order_id.invoice_ids[0].invoice_line_ids.filter(lambda a: a.return_line_id == doc.id).account_id.name if doc.order_id.invoice_ids else "", body_format)
            col += 1
            ...
            if doc.order_id.invoice_ids:
                sheet.write(row, col,
                            str(doc.order_id.invoice_ids[0].invoice_date.strftime('%Y/%m/%d')) if doc.order_id.invoice_ids[
            ...
            col += 1
            sheet.write(row, col, doc.order_id.p_order_id.partner_ref, body_format)
            col += 1
            ...
            col += 1
            sheet.write(row, col, doc.purchase_order_id.unit_qty, body_format)
            col += 1
            sheet.write(row, col, doc.product_uom.name, body_format)
            col += 1
            sheet.write(row, col, doc.product_qty, body_format)
            col += 1
            sheet.write(row, col, doc.delivered_qty, body_format)
            col += 1
            ...
            col += 1
            sheet.write(row, col, (round(doc.price_subtotal + doc.price_tax, 2) - doc.get_invoice_discount()),
                        body_format)
            col += 1
            sheet.write(row, col, (doc.product_id.length * doc.purchase_order_id.unit_qty), body_format)
            col += 1
            sheet.write(row, col, (doc.product_id.length * doc.product_id.width * doc.purchase_order_id.unit_qty), body_format)
            col += 1
