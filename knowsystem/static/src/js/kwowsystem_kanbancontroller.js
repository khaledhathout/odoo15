...
const KnowSystemKanbanController = KanbanController.extend({
    ...
    */ 
    _renderSections: function () {
        ...
        self._rpc({
            ...
            var jsTreeAnchor = self.$('#sections').jstree(true);
            if (self.is_action_enabled("create")) {
            ...
            self.$('#sections').on("state_ready.jstree", self, function (event, data) {
                ...
                self.$('#sections').on("open_node.jstree", self, function (event, data) {
                    self._highlightParent(jsTreeAnchor, jsTreeAnchor.get_checked(), "#sections");
                });
            });
            defer.resolve();
        });
    ...
    */ 
    _renderTags: function () {
        ...
        self._rpc({
            ...
            var jsTreeAnchor = self.$('#tags').jstree(true);
            if (self.is_action_enabled("create")) {
            ...
            self.$('#tags').on("state_ready.jstree", self, function (event, data) {
                ...
                self.$('#tags').on("open_node.jstree", self, function (event, data) {
                    self._highlightParent(jsTreeAnchor, jsTreeAnchor.get_checked(), "#tags");
                });
            });
            defer.resolve();
        });
    ...
    */  
    _renderTypes: function () {
        ...
        self._rpc({
            ...
            var jsTreeAnchor = self.$('#know_types').jstree(true);
            if (self.is_action_enabled("delete")) {
            ...
            self.$('#know_types').on("state_ready.jstree", self, function (event, data) {
                ...
                self.$('#know_types').on("open_node.jstree", self, function (event, data) {
                    self._highlightParent(jsTreeAnchor, jsTreeAnchor.get_checked(), "#know_types");
                });
            });
            defer.resolve();
        });
    ...
    */
    _highlightParent(jsTreeAnchor, checkedNodes, jsSelector) {
        $(jsSelector + "* .knowsystem_selected_parent").removeClass("knowsystem_selected_parent");
        var allParentNodes = [];
        ...
        _.each(allParentNodes, function(nodeID) {
            $(jsSelector + " * .jstree-node#" + nodeID).addClass("knowsystem_selected_parent");
        });
    },  
    /**
     * @private
    ...
    */        
    _renderArticles: function () {
        ...
        if (refS) {
            ...
            this._highlightParent(refS, checkedSections, '#sections');
        };
        // tags domain            
        if (refT) {
            ...
            this._highlightParent(refT, checkedTags, '#tags');
        };
        // types domain 
        if (refTy) {
            ...
            this._highlightParent(refTy, checkedTypes, '#know_types');
        };
        return domain
    },
    /**
