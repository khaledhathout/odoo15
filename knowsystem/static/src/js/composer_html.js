...
const KnowSystemComposerHtml = FieldHtml.extend({
    ...
    */
    willStart: async function () {
        this.knowSystemOptionKey = this.model == "mail.compose.message" ? "composer" : "activity";
        await this._super();
        this.knowsystemTurn = await this._rpc({
            model: "knowsystem.section",
            method: "action_check_option",
            args: [this.knowSystemOptionKey],
        });
    },
    /**
    ...
    */
    _getWysiwygOptions: function () {
        ...
        options.knowSystemOptionKey = this.knowSystemOptionKey;
        return options;
    },
    /**
     * @private
    ...
    async _onApplyArticleAction(dialog, action) {
        ...
        if (article) {
            ...
            if (article.attachment_ids) {
                this._onAttachmentChange({ data: article.attachment_ids });
            };
        };
        dialog.close();
