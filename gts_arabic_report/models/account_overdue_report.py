...
class ReportOverdue(models.AbstractModel):
    ...
    def _get_account_move_lines(self, partner_ids):
        ...
        self.env.cr.execute("SELECT m.name AS move_id, l.date, l.invoice_id,l.name, l.ref, l.date_maturity, l.partner_id, l.blocked, l.amount_currency, l.currency_id, "
            ...
            "WHERE l.partner_id IN %s AND at.type IN ('receivable', 'payable') AND l.full_reconcile_id IS NULL GROUP BY l.date, l.name, l.invoice_id,l.ref, l.date_maturity, l.partner_id, at.type, l.blocked, l.amount_currency, l.currency_id, l.move_id, m.name ORDER BY l.date_maturity", (((fields.date.today(), ) + (tuple(partner_ids),))))
    ...
    def get_report_values(self, docids, data=None):
        ...
        for partner_id in docids:
            ...
            for line_tmp in lines[partner_id]:
                line = line_tmp.copy()
                invoice_line = self.env['account.invoice'].browse(line['invoice_id'])
                line['tag_ids'] = invoice_line.tag_ids
