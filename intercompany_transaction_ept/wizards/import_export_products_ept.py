...
class ImportExportProducts(models.TransientModel):
    ...
    def import_products_from_csv(self, ict, log_book):
        ...
        for line in reader:
            ...
            product = self.get_product_from_sku_barcode(default_code, barcode, log_book)
            if not product:
                continue
    ...
    def get_product_from_sku_barcode(self, default_code, barcode, log_book):
        """
        ...
        if not product:
            ...
            msg += "Default code is '%s'" % default_code
            if not default_code:
