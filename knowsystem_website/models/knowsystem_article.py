...
class knowsystem_article(models.Model):
    ...
    website_pinned = fields.Boolean(
        ...
        default=False,
        help="""If checked, such an article would be shown always above all articles and disregarding active page, which 
sections or tags are selected.""",
    ...
    def write(self, values):
         ...
         * we should update each arcticle for a real change to avoid writing 'bad' description to articles which are not
           updated (especially from documentation page)
        """
        res = True
