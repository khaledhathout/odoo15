...
async function highlightParent(jstreeObject, checkedNodes, jsSelector) {
    if (checkedNodes && checkedNodes != "" && checkedNodes.length != 0) {
        ...
        await _.each(allParentNodes, function(nodeID) {
            ...
            if (parentNode.li_attr && parentNode.li_attr.class) {
                if (parentNode.li_attr.class.indexOf("knowsystem_selected_parent") == -1) {
                    parentNode.li_attr.class += " knowsystem_selected_parent";
                };
            }
            else {
                parentNode.li_attr.class = "knowsystem_selected_parent";
    ...
    start: function () {
        ...
        $('#knowsystem_sections').each(function (index) {
            ...
            self.on("state_ready.jstree", self, function (event, data) {
               ...
               addJSTreeToolTip();
               highlightParent(jsTreeRef, checkedSectionsIDS, '#knowsystem_sections')
            });
        });
    },
});
    ...
    start: function () {
        ...
        $('#knowsystem_tags').each(function (index) {
            ...
            self.on("state_ready.jstree", self, function (event, data) {
               ...
               addJSTreeToolTip();
               highlightParent(jsTreeRefT, checkedTagsIDS, '#knowsystem_tags')
            });
        });
    },
});
...
publicWidget.registry.typesHierarchy = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        $('#knowsystem_types').each(function (index) {
            ...
            self.on("state_ready.jstree", self, function (event, data) {
               var jsTreeRefTy = self.jstree(true);
               jsTreeRefTy.check_node(checkedTypesIDS);
               var allTSelected = jsTreeRefTy.get_checked();
               var diff = allTSelected.filter(x => checkedTypesIDS.indexOf(x) < 0);
               jsTreeRefTy.uncheck_node(diff);
               self.on("check_node.jstree uncheck_node.jstree", self, function (event, data) {
                    reloadArticlesTypes(self);
               });
               highlightParent(jsTreeRefTy, checkedTypesIDS, '#knowsystem_types')
            });
        });
    },
});
