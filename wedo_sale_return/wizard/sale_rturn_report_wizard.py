...
class SaleReturnReportWizard(models.TransientModel):
    ...
    def get_report(self):
        ...
        return self.env.ref('wedo_sale_return.sale_return_report').report_action(self, data=data)
    ...
    def get_report_xlsx(self):
        ...
        return self.env.ref('wedo_sale_return.sale_return_report_xlsx').report_action(self, data=data)


class SaleReturnReport(models.AbstractModel):
    _name = 'report.wedo_sale_return.sale_return_template'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        docs = self.env['sale.return.line'].search([('date_order', '>=', from_date), ('date_order', '<=', to_date),
                                                    ('order_id.state', '=', 'done'),
...
class SaleReturnReportXLSX(models.AbstractModel):
    _name = 'report.wedo_sale_return.sale_return_template_xlsx'
    _inherit = "report.report_xlsx.abstract"
    _description = "Sale Return XLSX Report"

    def generate_xlsx_report(self, workbook, data, partners):
        from_date = data['form']['from_date']
        to_date = data['form']['to_date']

        report_name = "Sale Return Report"
        ...
        sheet.merge_range('H8:K9', ("مردودات مبيعات اﻷصناف حسب رقم الصنف"), header_format3)
        ...
        sheet.write(12, 1, "أمر مردودات المبيعات", table_header_format)
        sheet.write(12, 2, "حالة امر مردودات المبيعات", table_header_format)
        ...
        sheet.write(12, 8, "تاريخ أمر مردودات المبيعات", table_header_format)
        ...
        for doc in docs:
            ...
            col += 1
            sheet.write(row, col, doc.order_id.invoice_ids[0].name if doc.order_id.invoice_ids else '', body_format)
            col += 1
            sheet.write(row, col, doc.order_id.invoice_ids[0].invoice_line_ids[
                0].account_id.name if doc.order_id.invoice_ids else "", body_format)
            col += 1
            ...
            col += 1
            if doc.order_id.invoice_ids:
                sheet.write(row, col,
                            str(doc.order_id.invoice_ids[0].invoice_date.strftime('%Y/%m/%d')) if
                            doc.order_id.invoice_ids[
                                0].invoice_date else "", body_format)
            else:
                sheet.write(row, col, "", body_format)
            col += 1
            sheet.write(row, col, doc.order_id.user_id.name, body_format)
            col += 1
            sheet.write(row, col, doc.order_id.sale_order_id.team_id.name, body_format)
            col += 1
            sheet.write(row, col, doc.order_id.partner_id.name, body_format)
            col += 1
            sheet.write(row, col, doc.order_id.sale_order_id.client_order_ref, body_format)
            col += 1
            sheet.write(row, col, doc.product_id.categ_id.name, body_format)
            col += 1
            if doc.order_id.picking_ids:
                sheet.write(row, col, doc.order_id.picking_ids[0].location_id.name, body_format)
            else:
                sheet.write(row, col, "", body_format)
            col += 1
            ...
            col += 1
            sheet.write(row, col, doc.qty_return, body_format)
            col += 1
            sheet.write(row, col, doc.received_qty, body_format)
            col += 1
            sheet.write(row, col, doc.invoiced_qty, body_format)
            col += 1
            sheet.write(row, col, doc.sale_order_id.lot_id.name if doc.sale_order_id.lot_id else '', body_format)
            col += 1
            sheet.write(row, col, doc.price_unit, body_format)
            col += 1
            sheet.write(row, col, (doc.price_unit * doc.qty_return), body_format)
            col += 1
            sheet.write(row, col, round(doc.price_unit * doc.qty_return, 2) - doc.price_subtotal, body_format)
            col += 1
            ...
            col += 1
            sheet.write(row, col,
                        (doc.order_id.invoice_ids[0].invoice_line_ids[0].price_unit *
                         doc.order_id.invoice_ids[0].invoice_line_ids[0].quantity) if doc.order_id.invoice_ids else "",
                        body_format)
            col += 1
            sheet.write(row, col, (
                    round(doc.order_id.invoice_ids[0].invoice_line_ids[0].price_unit *
                          doc.order_id.invoice_ids[0].invoice_line_ids[0].quantity, 2) -
                    doc.order_id.invoice_ids[0].invoice_line_ids[0].price_subtotal) if doc.order_id.invoice_ids else "",
                        body_format)
            col += 1
            sheet.write(row, col, doc.order_id.invoice_ids[0].invoice_line_ids[0].price_subtotal if doc.order_id.invoice_ids else "",
                        body_format)
            col += 1
            sheet.write(row, col, (sum((doc.order_id.invoice_ids[0].invoice_line_ids[0].price_subtotal * tax.amount) for tax in
                                       doc.order_id.invoice_ids[0].invoice_line_ids[0].tax_ids) / 100) if doc.order_id.invoice_ids else "",
                        body_format)
            col += 1
            sheet.write(row, col, (round(doc.order_id.invoice_ids[0].invoice_line_ids[0].price_subtotal, 2) + (sum(
                round(doc.order_id.invoice_ids[0].invoice_line_ids[0].price_subtotal * tax.amount, 2) for tax in
                doc.order_id.invoice_ids[0].invoice_line_ids[0].tax_ids) / 100)) if doc.order_id.invoice_ids else "", body_format)
            index += 1
            row += 1
