...
class wizard_inventory_valuation_mrp(models.TransientModel):
    _name = 'wizard.inventory.valuation.mrp'
    ...
    def print_report(self):
        ...
        return self.env.ref('eq_inventory_valuation_report_mrp.action_inventory_valuation_mrp_template').report_action(self, data=datas)
    ...
    def print_xls_report(self):
        self.check_date_range()
        xls_filename = 'inventory valuation report mrp.xlsx'
        workbook = xlsxwriter.Workbook('/tmp/' + xls_filename)
        report_stock_inv_obj = self.env['report.eq_inventory_valuation_report_mrp.valuation_report']
        ...
        for warehouse in self.warehouse_ids:
            ...
            if not self.location_ids:
                ...
                worksheet.merge_range(9,13,9,14, "Manufacturing", header_merge_format)
                worksheet.merge_range(9,15,9,16, "Raw Material", header_merge_format)
                worksheet.merge_range(9,17,9,18, "Ending", header_merge_format)
                ...
                worksheet.write(10,15, "Qty", header_merge_format)
                worksheet.write(10,16, "Value", header_merge_format)
                worksheet.write(10,17, "Qty", header_merge_format)
                worksheet.write(10,18, "Value", header_merge_format)

                rows = 11
                prod_beginning_qty = prod_qty_in = prod_qty_out = prod_qty_int = prod_qty_adjust = prod_mrp_qty = prod_consume_qty = prod_ending_qty = 0.00
                prod_beginning_qty_val = prod_qty_in_val = prod_qty_out_val = prod_qty_int_val = prod_qty_adjust_val  = prod_qty_mrp_val = prod_qty_consume_val = prod_ending_qty_val = 0.00
                if not self.group_by_categ:
                    for product in report_stock_inv_obj._get_products(self):
                        ps = report_stock_inv_obj._get_products(self)
                        print("################### report_stock_inv_obj._get_products(self)",len(ps))
                        qty = report_stock_inv_obj.get_product_valuation(self,ps,45,warehouse,'mrp_production')
                        print("################### qty",qty)
                        print("################### ",dsds)
                        ...
                        product_qty_mrp = product_val.get('product_qty_mrp')
                        product_qty_mrp_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_mrp,warehouse,'mrp_production')
                        product_qty_consume = product_val.get('product_qty_consume')
                        product_qty_consume_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_consume,warehouse,'mrp_consume')

                        ending_qty = beginning_qty + product_qty_in + product_qty_out\
                                    + product_qty_internal + product_qty_adjustment + product_qty_mrp + product_qty_consume
                        ...
                        worksheet.write(rows, 13, product_qty_mrp, header_data_format)
                        worksheet.write(rows, 14, formatLang(self.env, product_qty_mrp_val), header_data_format)
                        worksheet.write(rows, 15, abs(product_qty_consume), header_data_format)
                        worksheet.write(rows, 16, formatLang(self.env, abs(product_qty_consume_val)), header_data_format)

                        worksheet.write(rows, 17, ending_qty, header_data_format)
                        worksheet.write(rows, 18, formatLang(self.env, ending_qty_val), header_data_format)
                        ...
                        prod_mrp_qty += product_qty_mrp
                        prod_qty_mrp_val += product_qty_mrp_val
                        prod_consume_qty += product_qty_consume
                        prod_qty_consume_val += product_qty_consume_val
                    ...
                    worksheet.write(rows + 1, 13, prod_mrp_qty, header_merge_format)
                    worksheet.write(rows + 1, 14, formatLang(self.env, prod_qty_mrp_val), header_merge_format)
                    worksheet.write(rows + 1, 15, abs(prod_consume_qty), header_merge_format)
                    worksheet.write(rows + 1, 16, formatLang(self.env, abs(prod_qty_consume_val)), header_merge_format)


                    worksheet.write(rows + 1, 17, prod_ending_qty, header_merge_format)
                    worksheet.write(rows + 1, 18, formatLang(self.env, prod_ending_qty_val), header_merge_format)

                else:
                    rows += 1
                    product_val = report_stock_inv_obj.get_product_sale_qty(self, warehouse)
                    for categ, product_value in product_val.items():
                        categ_prod_beginning_qty = categ_prod_qty_in = categ_prod_qty_out = categ_prod_qty_int = categ_prod_qty_adjust  = categ_prod_qty_mrp = categ_prod_qty_consume = categ_prod_ending_qty = 0.00
                        categ_prod_beginning_qty_val = categ_prod_qty_in_val = categ_prod_qty_out_val = categ_prod_qty_int_val = categ_prod_qty_adjust_val = categ_prod_qty_mrp_val = categ_prod_qty_consume_val = categ_prod_ending_qty_val = 0.00
                        worksheet.merge_range(rows, 0, rows, 18, self.env['product.category'].browse(categ).name, header_merge_format)
                        rows += 1
                        for product in product_value:
                            ...
                            product_qty_mrp = product.get('product_qty_mrp')
                            product_qty_mrp_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_mrp,warehouse,'mrp_production')
                            product_qty_consume = product.get('product_qty_consume')
                            product_qty_consume_val = report_stock_inv_obj.get_product_valuation(self,product_id,product_qty_consume,warehouse,'mrp_consume')

                            ending_qty = beginning_qty + product_qty_in + product_qty_out + product_qty_internal + product_qty_adjustment + product_qty_mrp + product_qty_consume
                            ...
                            categ_prod_qty_mrp += product_qty_mrp
                            categ_prod_qty_consume += product_qty_consume
                            ...
                            categ_prod_qty_mrp_val += product_qty_mrp_val
                            categ_prod_qty_consume_val += product_qty_consume_val
                        ...
                        worksheet.write(rows, 13, categ_prod_qty_mrp, header_merge_format)
                        worksheet.write(rows, 14, formatLang(self.env, categ_prod_qty_mrp_val), header_merge_format)
                        worksheet.write(rows, 15, abs(categ_prod_qty_consume), header_merge_format)
                        worksheet.write(rows, 16, formatLang(self.env, abs(categ_prod_qty_consume_val)), header_merge_format)
                        worksheet.write(rows, 17, categ_prod_ending_qty, header_merge_format)
                        worksheet.write(rows, 18, formatLang(self.env, categ_prod_ending_qty_val), header_merge_format)
                        ...
                        prod_mrp_qty += categ_prod_qty_mrp
                        prod_consume_qty += categ_prod_qty_consume
                        ...
                        prod_qty_mrp_val += categ_prod_qty_mrp_val
                        prod_qty_consume_val += categ_prod_qty_consume_val
                ...
                worksheet.merge_range(9,14,9,15, "Manufacturing", header_merge_format)
                worksheet.merge_range(9,16,9,17, "Raw Material", header_merge_format)
                worksheet.merge_range(9,18,9,19, "Ending", header_merge_format)
                ...
                worksheet.write(10,16, "Qty", header_merge_format)
                worksheet.write(10,17, "Value", header_merge_format)
                worksheet.write(10,18, "Qty", header_merge_format)
                worksheet.write(10,19, "Value", header_merge_format)
                ...
                if not self.group_by_categ:
                    for product in report_stock_inv_obj._get_products(self):
                        ...
                        product_qty_mrp = location_wise_data[1][5]
                        product_qty_mrp_val = report_stock_inv_obj.get_product_valuation(self,product,product_qty_mrp,warehouse,'mrp_production')
                        ...
                        worksheet.write(rows, 14, product_qty_mrp, header_merge_format)
                        worksheet.write(rows, 15, formatLang(self.env, product_qty_mrp_val), header_merge_format)
                        worksheet.write(rows, 16, abs(product_qty_consume), header_merge_format)
                        worksheet.write(rows, 17, formatLang(self.env, abs(product_qty_consume_val)), header_merge_format)

                        worksheet.write(rows, 18, ending_qty, header_merge_format)
                        worksheet.write(rows, 19, formatLang(self.env, ending_qty_val), header_merge_format)
                        rows += 1
                        ...
                        for location, value in location_wise_data[0].items():
                            ...
                            worksheet.write(rows, 9, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,abs(value[2]),warehouse,'out')), header_data_format)
                            ...
                            worksheet.write(rows, 15, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[5],warehouse,'mrp_production')), header_data_format)
                            worksheet.write(rows, 16, abs(value[6]), header_data_format)
                            worksheet.write(rows, 17, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,abs(value[6]),warehouse,'mrp_consume')), header_data_format)
                            worksheet.write(rows, 18, value[7], header_data_format)
                            worksheet.write(rows, 19, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product,value[7],warehouse,'final')), header_data_format)
                            rows += 1
                    ...
                    worksheet.write(rows, 14, prod_mrp_qty, header_merge_format)
                    worksheet.write(rows, 15, formatLang(self.env, prod_qty_mrp_val), header_merge_format)
                    worksheet.write(rows, 16, abs(prod_consume_qty), header_merge_format)
                    worksheet.write(rows, 17, formatLang(self.env, abs(prod_qty_consume_val)), header_merge_format)
                    worksheet.write(rows, 18, prod_ending_qty, header_merge_format)
                    worksheet.write(rows, 19, formatLang(self.env, prod_ending_qty_val), header_merge_format)
                    ...
                    for categ, product_value in product_val.items():
                        categ_prod_beginning_qty = categ_prod_qty_in = categ_prod_qty_out = categ_prod_qty_int = categ_prod_qty_adjust = categ_prod_qty_mrp = categ_prod_qty_consume = categ_prod_ending_qty = 0.00
                        categ_prod_beginning_qty_val = categ_prod_qty_in_val = categ_prod_qty_out_val = categ_prod_qty_int_val = categ_prod_qty_adjust_val = categ_prod_qty_mrp_val = categ_prod_qty_consume_val = categ_prod_ending_qty_val = 0.00
                        worksheet.merge_range(rows, 0, rows, 19, self.env['product.category'].browse(categ).name, header_merge_format)
                        rows += 1
                        for product in product_value:
                            product_id = self.env['product.product'].browse(product['product_id'])
                            ...
                            product_qty_consume_val = report_stock_inv_obj.get_product_valuation(self,product_id,abs(product_qty_consume),warehouse,'mrp_consume')
                            ending_qty = location_wise_data[1][7]
                            ending_qty_val = report_stock_inv_obj.get_product_valuation(self,product_id,ending_qty,warehouse,'final')
                            ...
                            for location, value in location_wise_data[0].items():
                                ...
                                worksheet.write(rows, 8, abs(value[2]), header_data_format)
                                worksheet.write(rows, 9, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,abs(value[2]),warehouse,'out')), header_data_format)
                                worksheet.write(rows, 10, value[3], header_data_format)
                                ...
                                worksheet.write(rows, 14, value[5], header_data_format)
                                worksheet.write(rows, 15, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[5],warehouse,'mrp_production')), header_data_format)
                                worksheet.write(rows, 16, abs(value[6]), header_data_format)
                                worksheet.write(rows, 17, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,abs(value[6]),warehouse,'mrp_consume')), header_data_format)
                                worksheet.write(rows, 18, value[7], header_data_format)
                                worksheet.write(rows, 19, formatLang(self.env, report_stock_inv_obj.get_product_valuation(self,product_id,value[7],warehouse,'final')), header_data_format)
                                rows += 1
                        ...
                        worksheet.write(rows, 14, categ_prod_qty_mrp, header_merge_format)
                        worksheet.write(rows, 15, formatLang(self.env, categ_prod_qty_mrp_val), header_merge_format)
                        worksheet.write(rows, 16, abs(categ_prod_qty_consume), header_merge_format)
                        worksheet.write(rows, 17, formatLang(self.env, abs(categ_prod_qty_consume_val)), header_merge_format)
                        
                        worksheet.write(rows, 18, categ_prod_ending_qty, header_merge_format)
                        worksheet.write(rows, 19, formatLang(self.env, categ_prod_ending_qty_val), header_merge_format)
                        rows += 1
                        rows += 2
