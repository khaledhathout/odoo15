...
class SaasOdooVersion(models.Model):
    ...
    def get_default_saas_values(self):
        ...
        data['odoo_version'] = self.env['saas.odoo.version'].sudo().search([('state', '=', 'confirm')]) if data['is_odoo_version'] else self.env['saas.odoo.version'].sudo().search([('state', '=', 'confirm'), ('code', '=', '15.0')], limit=1)
