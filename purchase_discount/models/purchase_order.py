...
class PurchaseOrder(models.Model):
    ...
    def _onchange_is_free_product(self):
        for rec in self.order_line:
            rec._onchange_is_free_product()
...
class PurchaseOrderLine(models.Model):
    ...
    @api.onchange('product_id','discount','discount_fixed')
    def _onchange_is_free_product(self):
        """ If product is free make discount 100% """
        if self.product_id.is_free:
            self.discount = 100.0

    @api.depends("product_qty", "discount", "price_unit", "taxes_id", "discount_fixed")
    def _compute_amount(self):
        vals = {}
    ...
    def _get_discounted_price_unit(self):
        ...
        self.ensure_one()
        # if self.discount_fixed:
        #     return self.price_unit - (self.discount_fixed or 0.0)
        # else:
        return self.price_unit * (1 - self.discount / 100 or 0.0) - (self.discount_fixed or 0.0)
