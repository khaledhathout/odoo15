...
class StockCardReportWizard(models.TransientModel):
    ...
    location_id = fields.Many2many(comodel_name="stock.location", string="Location")
    product_ids = fields.Many2many(comodel_name="product.product", string="Products")
    category_id = fields.Many2one("product.category", string="Category")
    report_type = fields.Selection([('with_details', 'With Details'), ('without_details', 'Without Details')], string="Report Type", default="with_details")
    ...
    def _prepare_stock_card_report(self):
        ...
        if self.product_ids:
            ...
            if self.category_id:
                product_ids = self.env["product.product"].search([('categ_id','=', self.category_id.id)]).ids
            else:
        ...
        return {
            ...
            "location_id": [(6, 0, self.location_id.ids if self.location_id else self.env["stock.location"].search([("usage","=","internal")]).ids)],
