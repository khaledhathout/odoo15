...
class KSGlobalDiscountPurchases(models.Model):
    ...
    ks_global_discount_rate = fields.Float('Universal Discount', readonly=True,
                                           states={'draft': [('readonly', False)], 'sent': [('readonly', False)]})
    ks_amount_discount = fields.Monetary(string='Universal Discount', readonly=True, compute='ks_calculate_discount',
    ...
    @api.depends('order_line.price_subtotal','ks_global_discount_type','ks_global_discount_rate')
    def ks_calculate_discount(self):
        for rec in self:
            ...
            if rec.order_line :
                ...
                if rec.order_line.filtered(lambda x: x.taxes_id):
                    rec.order_line.filtered(lambda x: x.taxes_id)[0].global_discount =True
    ...
    def  _compute_tax_totals_json(self):
        def compute_taxes(order_line):
            if order_line.global_discount:
                return order_line.taxes_id._origin.with_context(discount=order_line.order_id.ks_amount_discount).compute_all(**order_line._prepare_compute_all_values())
    ...
    def _compute_amount(self):
        for line in self:
            ...
            if line.global_discount:
                taxes = line.taxes_id.with_context(discount=line.order_id.ks_amount_discount).compute_all(**line._prepare_compute_all_values())
            else:
