...
def create_get_stock_data_sp(cr):
    query = """
        ...
        CREATE OR REPLACE FUNCTION public.get_stock_data(
            ...
            IN start_date date,
            IN end_date date)
          RETURNS TABLE(company_id integer, company_name character varying, product_id integer, 
            product_name character varying, product_code character varying,product_category_id integer, 
            category_name character varying, warehouse_id integer, warehouse_name character varying, product_qty numeric, 
            valuation numeric, type text, stock_move_id integer, valuation_layer_id integer) AS
        $BODY$
        DECLARE 
            source_usage text;
            dest_usage text;
        BEGIN
            ...
            Select 
                ...
                -- (select sum(value) From stock_valuation_layer sv where sv.stock_move_id = T.stock_move_id) as valuation,
                T.valuation as valuation,
                transaction_type,
            ...
            (
                Select
                    ...
                    case when layer.stock_landed_cost_id IS NULL then
                        move.product_uom_qty else 0 end as product_qty,
                    layer.value as valuation,
            ...
            group by T.company_id, T.product_id, T.category_id, T.warehouse_id, T.company_name, T.product_code, T.product_name,T.category_name, T.warehouse_name, T.valuation, T.stock_move_id, T.layer_id
            ;
        END; $BODY$
...
def create_get_products_stock_movements_sp(cr):
    ...
    CREATE OR REPLACE FUNCTION public.get_products_stock_movements(
      ...
      RETURNS TABLE(company_id integer, company_name character varying ,product_id integer, product_name character varying, 
        product_code character varying,product_category_id integer, category_name character varying, warehouse_id integer, warehouse_name character varying, 
        opening_stock numeric, opening_stock_valuation Numeric,sales numeric, sales_valuation numeric,sales_return numeric, sales_return_valuation numeric,purchase numeric, purchase_valuation numeric,
        purchase_return numeric, purchase_return_valuation numeric ,internal_in numeric, internal_in_valuation Numeric,internal_out numeric, internal_out_valuation Numeric,
        adjustment_in numeric, adjustment_in_valuation Numeric,adjustment_out numeric, adjustment_out_valuation Numeric,production_in numeric, production_in_valuation Numeric,
        production_out numeric, production_out_valuation Numeric,transit_in numeric, transit_in_valuation Numeric,transit_out numeric, transit_out_valuation Numeric,
        valuation numeric, adjustment_valuation numeric, closing numeric) AS
    $BODY$
        DECLARE
            tr_start_date date;
            tr_end_date date;
    ...
    BEGIN   
        ...
        CREATE TEMPORARY TABLE transaction_table(
            ...
            opening_stock_valuation Numeric DEFAULT 0,
            sales Numeric DEFAULT 0,
            sales_valuation Numeric DEFAULT 0,
            sales_return numeric DEFAULT 0,
            sales_return_valuation numeric DEFAULT 0,
            purchase Numeric DEFAULT 0,
            purchase_valuation Numeric DEFAULT 0,
            purchase_return numeric DEFAULT 0,
            purchase_return_valuation numeric DEFAULT 0,
            internal_in Numeric DEFAULT 0,
            internal_in_valuation Numeric DEFAULT 0,
            internal_out Numeric DEFAULT 0,
            internal_out_valuation Numeric DEFAULT 0,
            adjustment_in Numeric DEFAULT 0,
            adjustment_in_valuation Numeric DEFAULT 0,
            adjustment_out Numeric DEFAULT 0,
            adjustment_out_valuation Numeric DEFAULT 0,
            production_in Numeric DEFAULT 0,
            production_in_valuation Numeric DEFAULT 0,
            production_out Numeric DEFAULT 0,
            production_out_valuation Numeric DEFAULT 0,
            transit_in Numeric DEFAULT 0,
            transit_in_valuation Numeric DEFAULT 0,
            transit_out Numeric DEFAULT 0,
            transit_out_valuation Numeric DEFAULT 0,
            closing Numeric DEFAULT 0,
            valuation Numeric DEFAULT 0,
            adjustment_valuation Numeric DEFAULT 0
        );
    
        IF start_Date is not null then 
            tr_start_date := '1900-01-01';
            tr_end_date := start_date - interval '1 day';
            Insert into transaction_table(company_id, company_name, product_id, product_name, product_code ,product_category_id, category_name, 
                warehouse_id, warehouse_name, opening_stock, opening_stock_valuation)
            select T.company_id, T.company_name, T.product_id, T.product_name, T.product_code,T.product_category_id, T.category_name, 
            T.warehouse_id, T.warehouse_name, T.opening_stock, T.op_stock_valuation
            from get_products_opening_stock(company_ids, product_ids, category_ids, warehouse_ids, tr_start_date, tr_end_date) T;
        END IF;
    
        -- Sales
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code,product_category_id, 
            category_name, warehouse_id, warehouse_name, sales, transaction_type,valuation)
        select T.company_id, T.company_name, T.product_id, T.product_name, T.product_code, T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty, T.type, T.valuation
        from get_stock_data(company_ids, product_ids, category_ids, warehouse_ids, 'sales', start_date, end_date)T;
    
        -- Sales Return
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code,product_category_id, 
            category_name, warehouse_id, warehouse_name, sales_return, transaction_type, valuation)
        select T.company_id, T.company_name, T.product_id, T.product_name, T.product_code , T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty, T.type, T.valuation
        ...
        -- Purchase 
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code, product_category_id, 
            category_name, warehouse_id, warehouse_name, purchase, transaction_type, valuation)
        select  T.company_id, T.company_name, T.product_id, T.product_name, T.product_code ,T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty, T.type, T.valuation
        ...
        -- Purchase Return
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code, product_category_id, 
            category_name, warehouse_id, warehouse_name, purchase_return, transaction_type, valuation)
        select  T.company_id, T.company_name, T.product_id, T.product_name, T.product_code , T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty,  T.type, T.valuation
        ...
        -- Internal IN
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code, product_category_id, 
            category_name, warehouse_id, warehouse_name, internal_in, transaction_type, valuation)
        select  T.company_id, T.company_name, T.product_id, T.product_name, T.product_code , T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty,T.type, T.valuation
        ...
        -- Internal Out 
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code, product_category_id, 
            category_name, warehouse_id, warehouse_name, internal_out, transaction_type, valuation)
        select  T.company_id, T.company_name, T.product_id, T.product_name, T.product_code , T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty, T.type, T.valuation
        ...
        -- Adjustment IN
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code, product_category_id, 
            category_name, warehouse_id, warehouse_name, adjustment_in, transaction_type, valuation)
        select T.company_id, T.company_name, T.product_id, T.product_name, T.product_code , T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty, T.type, T.valuation
        ...
        -- Adjustment Out 
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code, product_category_id, 
            category_name, warehouse_id, warehouse_name, adjustment_out, transaction_type, valuation)
        select  T.company_id, T.company_name, T.product_id, T.product_name, T.product_code , T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty, T.type, T.valuation
        ...
        -- Production IN
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code, product_category_id, 
            category_name, warehouse_id, warehouse_name, production_in, transaction_type, valuation)
        select  T.company_id, T.company_name, T.product_id, T.product_name, T.product_code , T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty, T.type, T.valuation
        ...
        -- Production Out 
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code, product_category_id, 
            category_name, warehouse_id, warehouse_name, production_out, transaction_type, valuation)
        select  T.company_id, T.company_name, T.product_id, T.product_name, T.product_code , T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty, T.type, T.valuation
        ...
        -- Transit IN
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code, product_category_id, 
            category_name, warehouse_id, warehouse_name, transit_in, transaction_type, valuation)
        select  T.company_id, T.company_name, T.product_id, T.product_name, T.product_code , T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty, T.type, T.valuation
        ...
        -- Transit Out 
        Insert into transaction_table(company_id, company_name, product_id, product_name, product_code, product_category_id, 
            category_name, warehouse_id, warehouse_name, transit_out, transaction_type, valuation)
        select  T.company_id, T.company_name, T.product_id, T.product_name, T.product_code , T.product_category_id, 
        T.category_name, T.warehouse_id, T.warehouse_name, product_qty, T.type, T.valuation
        from get_stock_data(company_ids, product_ids, category_ids, warehouse_ids, 'transit_out', start_date, end_date)T;

        -- -- Valuation
        -- Insert into transaction_table(valuation)
        -- select value
        -- from stock_valuation_layer s where s.company_id = (select T.company_id from transaction_table T) and s.product_id = (select T.product_id from transaction_table T);
        ...
        (
            Select 
                T.company_id, T.company_name, T.product_id, T.product_name, T.product_code ,T.product_category_id, T.category_name, T.warehouse_id, T.warehouse_name,
                sum(T.opening_stock) as opening_stock,
                sum(T.opening_stock_valuation) as opening_stock_valuation,
                sum(T.sales) as sales,
                sum(CASE WHEN T.transaction_type = 'sales' THEN T.valuation else 0 END) as sales_valuation,
                sum(T.sales_return) as sales_return,
                sum(CASE WHEN T.transaction_type = 'sales_return' THEN T.valuation else 0 END) as sales_return_valuation,
                sum(T.purchase) as purchase,
                sum(CASE WHEN T.transaction_type = 'purchase' THEN T.valuation else 0 END) as purchase_valuation,
                sum(T.purchase_return) as purchase_return,
                sum(CASE WHEN T.transaction_type = 'purchase_return' THEN T.valuation else 0 END) as purchase_return_valuation,
                sum(T.internal_in) as internal_in,
                sum(CASE WHEN T.transaction_type = 'internal_in' THEN T.valuation else 0 END) as internal_in_valuation,
                sum(T.internal_out) as internal_out,
                sum(CASE WHEN T.transaction_type = 'internal_out' THEN T.valuation else 0 END) as internal_out_valuation,
                sum(T.adjustment_in) as adjustment_in,
                sum(CASE WHEN T.transaction_type = 'adjustment_in' THEN T.valuation else 0 END) as adjustment_in_valuation,
                sum(T.adjustment_out) as adjustment_out,
                sum(CASE WHEN T.transaction_type = 'adjustment_out' THEN T.valuation else 0 END) as adjustment_out_valuation,
                sum(T.production_in) as production_in,
                sum(CASE WHEN T.transaction_type = 'production_in' THEN T.valuation else 0 END) as production_in_valuation,
                sum(T.production_out) as production_out,
                sum(CASE WHEN T.transaction_type = 'production_out' THEN T.valuation else 0 END) as production_out_valuation,
                sum(T.transit_in) as transit_in,
                sum(CASE WHEN T.transaction_type = 'transit_in' THEN T.valuation else 0 END) as transit_in_valuation,
                sum(T.transit_out) as transit_out,
                sum(CASE WHEN T.transaction_type = 'transit_out' THEN T.valuation else 0 END) as transit_out_valuation,
                (select sum (value) from stock_valuation_layer s where s.company_id = T.company_id and s.product_id = T.product_id and CAST(s.create_date AS DATE) <= end_date) as valuation,
                (select sum (value) from stock_valuation_layer s where s.company_id = T.company_id and s.product_id = T.product_id and CAST(s.create_date AS DATE) <= end_date and s.stock_move_id IS NULL ) as valuation_adjustment
               -- sales_valuation + sales_return_valuation + purchase_valuation + purchase_return_valuation + internal_in_valuation + internal_out_valuation + adjustment_in_valuation + adjustment_out_valuation + production_in_valuation + production_out_valuation + transit_out_valuation  as valuation

            From
                transaction_table T
            group by 
                T.company_id, T.company_name, T.product_id, T.product_name, T.product_code,
...
def create_opening_stock_sp(cr):
    query="""
        ...
        CREATE OR REPLACE FUNCTION public.get_products_opening_stock(
            ...
            IN tr_end_date date)
          RETURNS TABLE(company_id integer, company_name character varying, product_id integer, product_name character varying, 
            product_code character varying,product_category_id integer, category_name character varying, warehouse_id integer, 
            warehouse_name character varying, opening_stock numeric, op_stock_valuation numeric) AS
        $BODY$
        BEGIN
            RETURN QUERY
            Select 
                cmp_id, cmp_name, p_id, prod_name, prod_code,categ_id, cat_name, wh_id, ware_name, sum(product_qty) as op_stock, sum(valuation) as op_stock_valuation
            From
            (
                select 
                    T.company_id as cmp_id, T.company_name as cmp_name, 
                    T.product_id as p_id, T.product_name as prod_name, T.product_code as prod_code, 
                    T.product_category_id as categ_id, T.category_name as cat_name, 
                    T.warehouse_id as wh_id, T.warehouse_name as ware_name,  
                    (product_qty * -1) as product_qty, T.valuation
                ...
                select 
                    T.company_id as cmp_id, T.company_name, 
                    T.product_id as p_id, T.product_name, T.product_code as prod_code,
                    T.product_category_id as categ_id, T.category_name, 
                    T.warehouse_id as wh_id, T.warehouse_name, 
                    (product_qty * -1) as product_qty, T.valuation
            ...
            group by cmp_id, cmp_name, p_id, prod_name, prod_code,categ_id, cat_name, wh_id, ware_name;   
        END; $BODY$
          LANGUAGE plpgsql VOLATILE
          COST 100
          ROWS 1000;
...
def create_get_stock_age_data_sp(cr):
    query = """
        ...
        -- DROP FUNCTION public.inventory_stock_age_report(integer[], integer[], integer[], integer[], integer[]);
        CREATE OR REPLACE FUNCTION public.inventory_stock_age_report(
            IN company_ids integer[],
            IN product_ids integer[],
            IN category_ids integer[],
            IN warehouse_ids integer[],
            IN location_ids integer[])
        ...
        --          tmpl.name,
                ...
                and 1 = case when array_length(warehouse_ids,1) >= 1 then
                    case when move.warehouse_id = ANY(warehouse_ids) then 1 else 0 end
                    else 1 end
                and 1 = case when array_length(location_ids,1) >= 1 then
                    case when move.location_id = ANY(location_ids) then 1 else 0 end
                    else 1 end
...
def create_get_stock_age_breakdown_data_sp(cr):
    query = """
        -- DROP FUNCTION get_inventory_age_breakdown_data(integer[],integer[],integer[],integer,integer[],integer[]) 
